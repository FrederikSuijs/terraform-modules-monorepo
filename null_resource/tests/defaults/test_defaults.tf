...
resource "test_assertions" "syntax_check" {
...
data "null_data_source" "null_resource" {
  depends_on = [
...
resource "test_assertions" "null_data_source" {
  component = "null_data_source"

  equal "equal-name" {
    description = "equal name"
    got         = data.null_data_source.null_resource.outputs.name
    want        = local.name
  }

  equal "equal-boolean" {
    description = "equal boolean"
    got         = data.null_data_source.null_resource.outputs.boolean
    want        = local.name #intentional typo
  }
}
